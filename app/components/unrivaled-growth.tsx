...
export default function UnrivaledGrowth() {
  return (
    <section className="scroll-animation-wrapper generic-conatiner h-screen flex md:flex-col justify-center items-center gap-12 >md:gap-24 mt-[80px] >md:mt-[100px]">
      ...
      <div className="scroll-animation-title heading-h2-extrabold bg-neutral-white-base bg-clip-text text-transparent >md:w-full md:h-1/2 >md:pl-32 flex justify-center md:items-end text-center _gradient_1t585_1">
      ...
      <ul className="rolling-text-wrapper relative h-full w-9/12 >md:w-full >md:mr-32 invisible overflow-hidden z-10">
        {stats.map((item) => (
          <li
            key={item.stat}
            className="rolling-text inline-block absolute left-0 top-0 >md:top-1/2 >md:-translate-y-1/2 flex md:flex-col gap-4 items-center"
          >
            <p className="heading-h1-extrabold">{item.stat}</p>
            <p className="md:text-center >md:w-3/5">{item.description}</p>
            {item.cta && (
              <a
                href="https://www.growthops.asia/work"
                target="_self"
                rel=""
                className="scroll-text-cta rounded-[40px] bg-transparent hover:bg-neutral-white-base text-white hover:text-primary-pink-base body2-bold border border-neutral-white-base px-10 py-3 transition ease-out duration-300 inline-block absolute left-1/2 md:-translate-x-1/2 >md:left-0 -bottom-16 >md:-bottom-14 md:w-max"
              >
                View more results
              </a>
            )}
