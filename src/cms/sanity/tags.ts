/**
 * Sanity's cache tags, and the pure functions that read them.
 *
 * A module of its own, importing nothing — not the client, not GROQ, not
 * `next-sanity` — because three sides of the adapter need these and none of
 * them can reach the others: the repositories (which import the authenticated
 * client), the webhook parser, and the Studio schema, which would otherwise
 * pull `next-sanity` into the Studio bundle.
 *
 * Keep it that way. Anything here that needs an import belongs elsewhere.
 */
 ...
 *
 * Every read in a `*.repository.ts` goes through `sanityFetch` (see `live.ts`)
...
/** The document types in `schema.ts` that the site reads. */
